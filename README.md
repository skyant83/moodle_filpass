...
The course management page lists pending and failed certificate entries. For automatic entries, it combines the per-entry backoff timestamp with Moodle's configured scheduled-task state to show the next task run that can retry the certificate. Selecting **Try manual upload** disables `autoretry` under the same per-issue lock used by the upload workflow, then immediately performs the normal PDF generation and FilPass request. A failed manual attempt keeps `status` as `failed` and `nextretry` as `0`, allowing another manual attempt without returning control to the scheduled task. Managers can also select **Resume automatic retries** to return a manually stopped entry to the next scheduled retry run.
